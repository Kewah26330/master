...
bot = Client(
    ...
    bot_token=os.environ.get(
        "BOT_TOKEN", "7403953709:AAFVPP9OGBWuqWSTMcD-ZjrqdeD2Xw4L72Q"
    ),
    api_id=int(os.environ.get("API_ID", "22922437")),
    api_hash=os.environ.get("API_HASH", "993ba20fa7ce13ec05a9b59111224f70"),
)
auth_users = [
    int(chat)
    for chat in os.environ.get("AUTH_USERS", "6624826381").split(",")
    if chat != ""
]
...
async def upload(bot: Client, m: Message):
    ...
    #     cmd=f'yt-dlp -o "{name}.mp4" "{url}"'
        ...
        "adda247" in url
    ):  # and raw_text2 in ["144", "240", "360", "480", "720", "no"]:
        cmd = f'yt-dlp -o "{name}.mp4" "{url}"'
...
async def account_login(bot: Client, m: Message):
    ...
    if "unknown" in out[0][1]:
        ...
        await m.reply_document(document=f, caption=name)
        await r.delete(True)
        return
    ...
    for i in out:
        if "youtu" in url:
            ...
            buttons.append(
                InlineKeyboardButton(
                    i[1], callback_data=f"id:bestvideo[height<={x}][ext=mp4]"
                )
            )
        else:
